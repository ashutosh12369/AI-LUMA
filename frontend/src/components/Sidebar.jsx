...
export default function Sidebar() {
  ...
  const CollapsedRail = () => (
    <div className="hidden lg:flex flex-col items-center w-[56px] h-screen bg-[#0f172a] border-r border-white/[0.06] py-4 gap-1 shrink-0">
      <button
          ...
          return (
            <button
              ...
              className={`flex items-center justify-center w-9 h-9 rounded-xl transition-colors duration-150 border-none cursor-pointer
                ${isActive ? "bg-white/10 text-white" : "bg-transparent text-slate-500 hover:bg-white/[0.05] hover:text-slate-300"}`}
            >
      ...
      <div className="mt-auto">
        {userData && (
          <div className="relative">
            ...
            <span className="absolute -bottom-px -right-px w-2 h-2 bg-green-500 rounded-full border-[1.5px] border-[#0f172a] block" />
          </div>
        )}
      </div>
    </div>
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]">
        ...
        <span className="text-[10px] font-medium text-white bg-white/10 border border-white/10 px-2 py-0.5 rounded-full tracking-wide">
         {userData?.plan ?? "pro"}
        </span>
      ...
      <div className="px-4 pt-4 pb-1">
        <button
          onClick={handleCreateConversation}
          className="w-full flex items-center justify-center gap-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 rounded-xl py-[10px] border-none cursor-pointer transition-colors duration-150"
        >
          <Plus size={15} />
          New Chat
        </button>
        ...
        const renderChatItem = (chat) => {
          ...
          return (
            <div
              ...
              className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 border ${isActive ? "bg-white/10 border-transparent" : "bg-transparent border-transparent hover:bg-white/[0.02]"}`}
            >
              <div
                onClick={() => handleSelectConversation(chat)}
                className="flex-1 flex items-center gap-3 min-w-0"
              >
                <div className={`p-1.5 rounded-lg shrink-0 transition-colors ${isActive ? "bg-white/10 text-white" : "text-slate-500"}`}>
  ...
  !userData?.avatar || imageError ? (
    ...
    />
              ...
              <span className="absolute -bottom-px -right-px w-[9px] h-[9px] bg-green-500 rounded-full border-2 border-[#0f172a] block" />
            </div>
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-3.5 left-4 z-50 flex items-center justify-center w-8 h-8 rounded-lg bg-[#0f172a] border border-white/[0.06] text-slate-400 hover:text-slate-200 transition-colors duration-150 cursor-pointer"
      >
        <Menu size={16} />
      </button>
